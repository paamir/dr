...
SinglePage.LoadModal = function () {
    ...
    $.get(url,
        ...
        function (htmlPage) {
            ...
            //این چند خط برای ولیدیشن هستند و اگر نباشیند ولی دیشن سمت کابر اجرا نخواهد ش/
...
jQuery.validator.addMethod("FileExtension",
    function (value, element, params) {
        if (element.files.count > 0) {
            var ExtensionHasAccess = ["jpg", "jpeg", "png"]
            var type = element.files[0].type.split("/")[1];
            if (ExtensionHasAccess.includes(type))
                return true;
            else {
                return false;
            }
